...
export default class PgPromiseAdapter implements DatabaseConnection {
  ...
  connect (): void {
    ...
    PgPromiseAdapter.connection = pg(this.options)(`postgres://${DB_USERNAME}:${DB_PASSWORD}@${DB_HOST}:${DB_PORT}/${DB_DATABASE}`)
  }
  ...
  async commit (): Promise<void> {
    await PgPromiseAdapter.connection.tx(async (t: any) => {
      const transactions: any[] = []
      for (const statement of this.statements) {
        transactions.push(await t.none(statement.statement, statement.params))
      }
